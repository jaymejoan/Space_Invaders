...
public class mainMenu {
  ...
  boolean resetLevel = false, levelSelected = false;
  Score sObject;
  ...
  void redraw(int lives) {
    ...
    sObject.setCurrentScore(currentScore); //updates current score
    //println("main menu score: " + currentScore);
  ...
  boolean checkLevelSelected() {
    return levelSelected;
  }

  /** returns the selected level */
  int getLevel(boolean r) {
    resetLevel = r;
    if (resetLevel == true)
      return level = 0;
    else 
